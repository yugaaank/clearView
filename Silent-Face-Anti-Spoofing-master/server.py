...
class SpoofService:
    ...
    def analyze(self, frame: np.ndarray):
        ...
        # Estimate pose
        with repo_workdir():
            pose = self.predictor.estimate_pose(frame, bbox)
...
def issue_challenge():
    """Return a random challenge gesture."""
    challenge_id = str(uuid4())
    import random
    
    # Simple weighted choice or just random
    # gestures = ["turn_left", "turn_right", "smile", "blink"]
    # For now, let's stick to what we implemented: turn_left/turn_right (translation based)
    gestures = [
        {"gesture": "turn_left", "instruction": "Move to Left Side"},
        {"gesture": "turn_right", "instruction": "Move to Right Side"},
    ]
